...
abstract class BasePlaybackController<E : PlaybackEngine>(
    ...
    internal fun ensureSetup() {
        ...
        // System logic sync
        queue.syncShuffleMode(state.value.isShuffleModeEnabled)
        initJob?.cancel()
    ...
    open fun setEngine(engine: E) {
        ...
        updateState { it.copy(availableSubtitles = emptyList(), availableAudioTracks = emptyList()) }
        if (engine.type == EngineType.MPV) {
            MpvResourceManager.acquireLease(this)
        }
        
        // Volume & ReplayGain sync
        ...
        (engine as? VideoEngine)?.setScaleMode(state.value.scaleMode)
        ...
        engine.onSubtitlesChanged = { subtitles ->
            updateState { it.copy(availableSubtitles = subtitles) }
        }
        engine.onAudioTracksChanged = { tracks ->
            updateState { it.copy(availableAudioTracks = tracks) }
        }
    ...
    /**
     * Sets the shuffle mode enabled state.
     */
    fun setShuffleModeEnabled(enabled: Boolean) {
        scope.launch {
            if (state.value.isShuffleModeEnabled != enabled) {
                updateState { it.copy(isShuffleModeEnabled = enabled) }
                aman.playbackengine.enginecore.internal.persistence.PlaybackSettingsManager.saveShuffleMode(enabled)
                val currentUid = state.value.currentMedia?.uid
    ...
    override fun toggleShuffle() {
        scope.launch {
            ...
            currentEngine?.updatePlaylist(queue.items.value, newIndex)
            aman.playbackengine.enginecore.internal.persistence.PlaybackSettingsManager.saveShuffleMode(queue.isShuffleOn())
    ...
    override fun toggleRepeatMode() {
        ...
        aman.playbackengine.enginecore.internal.persistence.PlaybackSettingsManager.saveRepeatMode(nextMode)
    ...
    override fun setPlaybackSpeed(speed: Float) {
        ...
        aman.playbackengine.enginecore.internal.persistence.PlaybackSettingsManager.saveSpeed(speed)
        triggerSave(SaveLevel.METADATA)
    }
    ...
    override fun setPlaybackPitch(pitch: Float) {
        ...
        aman.playbackengine.enginecore.internal.persistence.PlaybackSettingsManager.savePitch(pitch)
        triggerSave(SaveLevel.METADATA)
    }
    ...
    override fun selectSubtitleTrack(trackId: String?) {
        (currentEngine as? VideoEngine)?.selectSubtitleTrack(trackId)
    }

    override fun selectAudioTrack(trackId: String) {
        (currentEngine as? VideoEngine)?.selectAudioTrack(trackId)
    }

    override fun setScaleMode(mode: ScaleMode) {
        updateState { it.copy(scaleMode = mode) }
        (currentEngine as? VideoEngine)?.setScaleMode(mode)
    }

    override fun setExternalSubtitle(uri: String) {
        scope.launch {
            val currentMedia = state.value.currentMedia ?: return@launch
            val currentUid = currentMedia.uid
            val position = currentEngine?.getCurrentPosition() ?: 0L
            val wasPlaying = state.value.isPlaying

            // 1. Update the queue with the new subtitle URI
            queue.updateItemByUid(currentUid) { it.copy(externalSubtitleUri = uri) }

            // 2. Find the index in the newly updated active list
            val newIdx = queue.items.value.indexOfFirst { it.uid == currentUid }.coerceAtLeast(0)

            // 3. Re-prepare the engine to force it to read the new MediaItem with the SubtitleConfiguration
            currentEngine?.prepare(queue.items.value, newIdx, position, autoPlay = wasPlaying)
        }
    }
